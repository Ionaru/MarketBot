...
const replaceQuotes = (text: string): string =>
    text.replaceAll("'", "").replaceAll('"', "");

export const guessSystemInput = async (input: string) =>
    guessUserInput(input, systems);

export const guessRegionInput = async (input: string) =>
    guessUserInput(input, regions, regionsFuse);

export const guessItemInput = async (input: string) =>
    guessUserInput(input, items, itemsFuse);

export const matchWithRegex = (
    possibility: IUniverseNamesDataUnit,
    regex: RegExp,
) =>
    possibility.name ? possibility.name.match(regex) || undefined : undefined;

export const guessUserInput = async (
    itemString: string,
    possibilitiesList: IUniverseNamesData,
    fuse?: Fuse<IUniverseNamesDataUnit>,
    raw = true,
    ...
    if (!Number.isNaN(possibleId)) {
        possibilities.push(
            ...possibilitiesList.filter(
                (possibility): boolean => possibility.id === possibleId,
            ),
        );
    ...
    const shortcut = Object.keys(shortcuts).find((shortcutText) =>
        shortcutText.match(regex),
    );
    ...
    possibilities.push(
        ...possibilitiesList.filter((possibility): boolean | void => {
            if (possibility.name) {
                const possibilityName = replaceQuotes(
                    possibility.name,
                ).toLowerCase();
                const possibilityParts = possibilityName.split(" ");
                return itemWords.every((part) =>
                    possibilityParts.includes(part),
                );
            }
        }),
    );
    ...
    if (possibilities.length === 0) {
        ...
        possibilities.push(
            ...possibilitiesList.filter(
                (possibility): RegExpMatchArray | undefined =>
                    matchWithRegex(possibility, regex),
            ),
        );
    }
            ...
            ...possibilitiesList.filter((possibility): boolean | void => {
                if (possibility.name) {
                    return possibility.name
                        .toUpperCase()
                        .includes(itemString.toUpperCase());
                }
            }),
        );
    }
    ...
    if (possibilities.length === 0 && fuse) {
        // Use Fuse to search (slow but fuzzy).
        const fuseGuess = fuse.search(itemString)[0]?.item as
            | IUniverseNamesDataUnit
            | undefined;
    ...
    if (!itemData.id && raw) {
        ...
        const guessResult = await guessUserInput(itemString, list, fuse, false);
        itemData = guessResult.itemData;
    }

    return { guess, id: false, itemData };
};

const filterUnpublishedTypes = async (
...
export const getGuessHint = (
    guessReturn: IGuessReturn,
    userInput: string,
): string => {
    let returnString = "";
...
export const getSelectedRegion = async (input: string, regionReply: string) => {
    ...
    if (input) {
        const guessResult = await guessRegionInput(input);
