...
export const createLineGraph = (
  ...
  `;
    ...
    const svg = d3n
        .createSVG(pageWidth, pageHeight)
        .append("g")
    ...
    const makeXGridlines = (height: number) =>
        d3
            .axisBottom(xScale)
            .ticks(data.length)
            .tickFormat(timeFormat as any)
    ...
    const makeYGridlines = (width: number) =>
        d3.axisLeft(yScale).ticks(10).tickSize(-width);

    // Set scope of X-axis.
    const startDate = new Date(data.at(-1)!.x);
    ...
    g.append("g").call(yAxis).call(makeYGridlines(graphWidth));

    g.selectAll("line").attr("stroke", textColor);

    g.selectAll("text").attr("fill", textColor);

    g.selectAll("path.domain").attr("stroke", textColor);
...
export const exportGraphImage = async (graph: any, path: string) => {
    ...
    const type = "png";
    const screenShotOptions = { path, type, viewport } as const;
