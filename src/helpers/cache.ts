...
export const checkAndUpdateCache = async () => {
    const { useCache, serverVersion }: IValidateCacheReturn =
        await validateCache();

    const newRegions = await cacheUniverse(
        useCache,
        "regions",
        fetchUniverseRegions,
    );
    const newSystems = await cacheUniverse(
        useCache,
        "systems",
        fetchUniverseSystems,
    );
    ...
    cacheDebug(
        `Next cache check in ${formatNumber(timeUntilNextNoon / 3_600_000, 2)} hours`,
    );
    setTimeout(() => {
        checkAndUpdateCache().catch((error: Error) => {
            process.stderr.write((error.stack as string) + "\n");
            process.stderr.write(
                "An error prevented a cache update, attempting to re-use the old cache\n",
            );
        });
    }, timeUntilNextNoon);
...
const validateCache = async (): Promise<IValidateCacheReturn> => {
    ...
    if (!serverStatus) {
        process.stderr.write(
            "Could not get EVE Online server status, using cache if possible\n",
        );
    ...
    if (serverStatus.server_version === serverVersion) {
        cacheDebug(
            `EVE Online server version matches saved version, using cache`,
        );
        return { serverVersion: serverStatus.server_version, useCache: true };
    }

    cacheDebug(
        `EVE Online server version does not match saved version (or there is no saved version), cache invalid`,
    );
    return { serverVersion: serverStatus.server_version, useCache: false };
};

const cacheUniverse = async (
    useCache: boolean,
    type: string,
    ...
    if (useCache) {
        ...
        if (cachedData) {
            ...
            try {
                ...
                cacheDebug(
                    `Loaded ${cachedNames.length} ${type} from cache into memory`,
                );
                return cachedNames;
            } catch {
...
export const checkAndUpdateCitadelCache = async (): Promise<void> => {
    ...
    setInterval(
        () => {
            void fetchCitadelData()
                .catch(() => ({}))
                .then((newCitadels) => {
                    if (
                        Object.keys(newCitadels).length > 0 &&
                        JSON.stringify(newCitadels) !== JSON.stringify(citadels)
                    ) {
