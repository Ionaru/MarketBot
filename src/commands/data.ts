...
const dataCommandLogic = async () => {
    ...
    reply +=
        `I have processed ${makeCode(commandCount)} ${commandsWord} in total, ` +
    ...
    reply +=
        `I am currently tracking ${makeCode(trackingCount)} item ${priceWord} for ` +
    ...
    if (client) {
        ...
        const currentServerWord = pluralize(
            "server",
            "servers",
            currentServerCount,
        );
