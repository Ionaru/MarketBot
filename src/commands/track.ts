    ...
    constructor(
        ...
        super(creator, {
            ...
            options: [
                ...
                {
                    description:
                        "The region to track the item price in. Default: The Forge",
                    name: "region",
                    required: false,
                    type: CommandOptionType.STRING,
                },
                {
                    description:
                        "The minimum amount the order needs to change for a notification. Default: 1.00 ISK",
                    name: "limit",
                    required: false,
...
export class ClearTrackingCommand extends SlashCommand {
    ...
    async run(context: CommandContext): Promise<void> {
        ...
        if (trackingEntries && trackingEntries.length > 0) {
            let entries = trackingEntries.filter(
                (trackingEntry) =>
                    trackingEntry.channel_id === context.channelID,
            );
            if (
                itemId &&
                itemData &&
...
const trackCommandLogic = async (
    context: CommandContext,
    type: "buy" | "sell",
): Promise<ITrackCommandLogicReturn> => {
    const maxEntries = 3;
    ...
    if (changeLimit < 0.01) {
        reply +=
            makeCode("0.01 ISK") +
            " is the minimum change amount I can handle.";
        reply += newLine();
        changeLimit = 0.01;
    }

    const trackingEntries: TrackingEntry[] = await TrackingEntry.find();
    const dupEntries = trackingEntries.filter(
        (trackingEntry) => trackingEntry.sender_id === context.user.id,
    );
    ...
    const channelItemDup = dupEntries.some(
        (duplicate) =>
            duplicate.item_id === itemData.id &&
            duplicate.region_id === selectedRegion.id &&
            duplicate.tracking_type === type &&
            duplicate.channel_id === context.channelID,
    );
    if (channelItemDup) {
    ...
    const originalOrder = await getCheapestOrder(
        type,
        itemData.id,
        selectedRegion.id,
    );
    ...
    reply +=
        `Started ${makeBold(type)} price tracking for ${itemFormat(itemData.name)} in ${regionFormat(regionName)}, ` +
...
export const performTrackingCycle = async () => {
    ...
    const entriesDone: Array<{
        entry: TrackingEntry;
        order?: IMarketOrdersDataUnit;
    }> = [];
    ...
    for (const entry of trackingEntries) {
        ...
        const duplicateEntry = entriesDone.find(
            (entryDone) =>
                entryDone.entry.item_id === entry.item_id &&
                entryDone.entry.region_id === entry.region_id,
        );

        const currentOrder =
            duplicateEntry && duplicateEntry.order
                ? duplicateEntry.order
                : await getCheapestOrder(
                      entry.tracking_type,
                      entry.item_id,
                      entry.region_id,
                  );
        ...
        if (
            currentOrder &&
            currentPrice &&
            currentPrice !== entry.tracking_price
        ) {
            ...
            const roundedChange = +(
                Math.round(Number(changeAbsolute.toString() + "e+" + "2")) +
                "e-" +
                2
            );

            if (roundedChange >= entry.tracking_limit) {
                await sendChangeMessage(
                    entry.channel_id,
                    currentOrder,
                    entry,
                    changeAbsolute,
...
const sendChangeMessage = async (
    channelId: string,
    currentOrder: IMarketOrdersDataUnit,
    entry: TrackingEntry,
    change: number,
) => {
    ...
    const droppedRoseWord = droppedRose(
        currentOrder.price - entry.tracking_price,
    );
    ...
    const regionName = regions.find(
        (region) => region.id === entry.region_id,
    )!.name;
