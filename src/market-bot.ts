...
export const activate = async () => {
    ...
    await checkAndUpdateCache().catch((error: Error) => {
        process.stderr.write(
            "Unable to create initial cache, bot cannot function!\n",
        );
        throw error;
    });
    ...
    if (token && typeof token === "string") {
        ...
        slashCreatorService.registerCommand(
            (slashCreator) => new PriceCommand(slashCreator),
        );
        slashCreatorService.registerCommand(
            (slashCreator) => new ItemCommand(slashCreator),
        );
        slashCreatorService.registerCommand(
            (slashCreator) => new DataCommand(slashCreator),
        );
        slashCreatorService.registerCommand(
            (slashCreator) => new InfoCommand(slashCreator),
        );
        slashCreatorService.registerCommand(
            (slashCreator) => new BuyOrdersCommand(slashCreator),
        );
        slashCreatorService.registerCommand(
            (slashCreator) => new SellOrdersCommand(slashCreator),
        );
        slashCreatorService.registerCommand(
            (slashCreator) => new HistoryCommand(slashCreator),
        );
        slashCreatorService.registerCommand(
            (slashCreator) => new TrackCommand(slashCreator, "buy"),
        );
        slashCreatorService.registerCommand(
            (slashCreator) => new TrackCommand(slashCreator, "sell"),
        );
        slashCreatorService.registerCommand(
            (slashCreator) => new TrackListCommand(slashCreator),
        );
        slashCreatorService.registerCommand(
            (slashCreator) => new ClearTrackingCommand(slashCreator),
        );
...
const finishActivation = () => {
    if (process.env.DISABLE_TRACKING_CYCLE !== "true") {
        void performTrackingCycle().then(() => {
            startTrackingCycle();
        });
    }
...
export const deactivate = (exitProcess: boolean, error = false): void => {
